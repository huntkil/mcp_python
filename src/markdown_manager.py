...
class MarkdownManager:
    ...
    def read_file(self, file_path: str, encoding: str = "utf-8") -> Dict[str, Any]:
        ...
        try:
            # Validate file path
            is_valid, sanitized_path = validate_file_path(
                file_path, str(self.base_path)
            )
            if not is_valid:
                return {
                    "success": False,
                    "error": f"Invalid file path: {sanitized_path}",
                }
            ...
            # Read file content
            with open(sanitized_path, encoding=encoding) as f:
                content = f.read()
    ...
    def create_file(
        ...
        try:
            ...
            if not ensure_directory_exists(sanitized_path):
                return {
                    "success": False,
                    "error": f"Failed to create directory for: {file_path}",
                }
    ...
    def delete_file(self, file_path: str, confirm: bool = False) -> Dict[str, Any]:  # noqa: ARG002
        """
    ...
    def list_files(
        self, directory: str = ".", recursive: bool = False, pattern: str = "*.md"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not is_valid:
                return {
                    "success": False,
                    "error": f"Invalid directory path: {sanitized_path}",
                }
    ...
    def search_content(
        ...
        try:
            ...
            markdown_files = list(search_path.rglob("*.md")) + list(
                search_path.rglob("*.markdown")
            )
    ...
    def manage_frontmatter(
        self, file_path: str, action: str, metadata: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            current_frontmatter, content_without_frontmatter = extract_frontmatter(
                content
            )
            ...
            if action == "get":
                ...
                if metadata is None:
                    return {
                        "success": False,
                        "error": "Metadata is required for 'set' action",
                    }
                ...
                new_content = add_frontmatter(
                    content_without_frontmatter, current_frontmatter
                )
                ...
                return {
                    "success": False,
                    "error": f"Invalid action: {action}. Valid actions: get, set, update, remove",
                }
